...
Future<SubscriptionDetailsModel> fetchPremiumDetails() async {
  http.Response response;
  //String deviceToken/f

  try {
    response = await http.post(
      Uri.parse(Urls.premiumDetailsUrl),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
      body: jsonEncode(<String, String>{
        'UserID': (await userData().getUserId()).toString(),
  ...
  if (response.statusCode == 200) {
    ...
    return SubscriptionDetailsModel.fromMap(jsonDecode(response.body));
  } else {
